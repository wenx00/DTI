...
logger = logging.getLogger('DTI_AutoML')
...
def main(args):
    ...
    for epoch in range(30):
        ...
        logger.debug('Epoch {} train finished, and takes {}s.'.format(epoch, time.time() - t0))
        # val model
        model.eval()
        # print('--' * 10)
        ...
        for input_nodes, positive_graph, negative_graph, blocks in tqdm.tqdm(valid_dataloader):
            ...
            logger.debug('valid accuracy %g', current_acu)
            logger.debug('Pipe send intermediate result done.')
            if epoch == 29:
                nni.report_final_result(current_acu)
                logger.debug('Final result is %g', current_acu)
                logger.debug('Send final result done.')
        # print('Epoch {} valid finished, and takes {}s.'.format(epoch, time.time() - t1))
        logger.debug('Epoch {} valid finished, and takes {}s.'.format(epoch, time.time() - t1))
